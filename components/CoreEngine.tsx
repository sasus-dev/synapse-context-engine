        ...
        <div className={`glass-card p-6 rounded-[2rem] border-2 transition-all duration-300 ${stage === 'security_blocked' ? 'border-red-600/40' : 'border-white/10'}`}>
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            <div className="flex-1 space-y-1.5">
              ...
              >
                {(Object.values(graph.nodes) as Node[]).filter(n => ['project', 'goal'].includes(n.type)).map(n => <option key={n.id} value={n.id}>{n.label}</option>)}
              </select>
            </div>
            <div className="flex-1 space-y-1.5">
              ...
              >
                ...
                <option value="Jarmo and Sarah need to decide on the API Framework by Friday. Budget is $10k.">Complex Clustering Test</option>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition-all cyber-button flex items-center justify-center gap-2 ${stage === 'security_blocked' ? 'bg-red-700' : 'bg-purple-600 hover:bg-purple-500 text-white shadow-xl shadow-purple-900/20'
                  }`}
              >
              ...
              <button
                ...
                className={`px-8 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 border-2 ${isPlaying
